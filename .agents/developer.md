...
routingDescription: 适用：小到中等规模直接实现、明确 bug 修复、按 reviewer 反馈修正、或无需 planner 的小闭环任务。不适用：纯调研、纯审查、预计涉及 6 个及以上文件或架构/验收不清的复杂规划、已有细粒度 tasks/todo+任务名.md 等待 implementer 执行。
context: fork
tools:
  - Read
  - Write
...
- 一旦创建或采用执行计划并开始实施，该计划就是本轮完成契约；final 前必须逐项核对 Slice / Step、验收、必跑检查、交付证据和 review 状态，未完成项存在时只能报告 blocker。
