---
name: "developer"
description: "需求明确的小到中等规模开发者。负责自己完成小范围规划和实现闭环。 适用：需求明确的小到中等规模直接实现、明确 bug 修复、按 reviewer 反馈修正、或无需 planner 的小闭环任务。不适用：纯调研、纯审查、预计涉及 6 个及以上文件或架构/验收不清的复杂规划、已有细粒度 tasks/todo+任务名.md 等待 implementer 执行。"
tools:
  - "Read"
  - "Write"
  - "Edit"
...
- Source routing: 适用：需求明确的小到中等规模直接实现、明确 bug 修复、按 reviewer 反馈修正、或无需 planner 的小闭环任务。不适用：纯调研、纯审查、预计涉及 6 个及以上文件或架构/验收不清的复杂规划、已有细粒度 tasks/todo+任务名.md 等待 implementer 执行。
