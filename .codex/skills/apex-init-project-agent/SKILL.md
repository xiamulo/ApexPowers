...
根 `AGENTS.md` / `CLAUDE.md` 固定模板必须内置“契约式小任务 + 运行时结构化分解 + 证据化验收”工作流，但它是按需严格流程，不是普通小任务默认负担。只有用户要求严格流程、预计跨 8 个以上文件、高风险改动或 PR 级交付时才强制 contract；普通小任务只要求读相关文件、最小改动、最小验证、汇报结果。该规则属于根级硬规则，不放到 `.claude/rules/` 才生效，也不得由模型在生成 rules 时自由发挥。修改该工作流时，必须同步更新 `scripts/init_project_agent.py` 中的 `CLAUDE_TEMPLATE`；如需 Codex 额外强调，再同步更新 `CODEX_RULE_APPENDIX`。模板中的 YAML contract 示例使用中文 key，例如 `任务ID`、`目标`、`范围`、`允许路径`、`禁止路径`、`验收`、`必跑检查`、`交付证据`、`需要审查`；如果未来接入脚本解析，这些 key 必须保持稳定。模板还必须明确：最小交付不是 MVP、第一版或部分 Slice；一旦输出执行计划或 contract 并开始执行，它就是本轮完成契约，final 前必须逐项核对对应验收、检查和证据。
