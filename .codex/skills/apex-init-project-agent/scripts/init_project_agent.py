...
- 增量优于大爆炸：优先小步、可理解、可维护的修改，而不是大规模高风险重写。
...
- 不绕过用户、任务或 CI 明确要求的门禁、pre-commit、hook 或 CI。
- 不修改生成规则、hook、CI、agent 配置等保护层来逃避门禁。
...
本节是 Codex 专用的压缩规则区。初始化或重新生成项目规则时，必须根据 `.claude/rules/api-design.md`、`.claude/rules/backend.md`、`.claude/rules/frontend.md` 回填下面三组内容；每组最多 10 行，只保留会直接影响代码结构、数据边界、错误处理、安全和兼容性的硬规则。对应领域在项目中不存在时，写明“不适用”及依据。
...
- 以最小交付为准，不擅自扩范围；最小交付指用最少改动完整满足用户明确验收，不代表只交 MVP、第一版或部分 Slice。除非用户明确同意拆版，否则计划中的所有验收项完成前不得 final；只能在阻塞时汇报 blocker。
- 不要写成命令式、审查式、技术汇报式语气。
...
- **需求回声**：非平凡实现前，先用一句通俗中文复述“我理解你真正要的是……”，并把结果写成可验证行为。低风险明确任务可复述后直接执行；如果同一句需求存在两个以上合理解释，或不同解释会影响交互模型、状态持久化、数据模型、权限、安全、范围边界或交付形式，先问用户确认，不要先写代码。
- **二次失败升级**：同一问题被用户反馈“不是这个意思 / 不对 / 还是不对 / 刷新后仍然 / 又出现 / 不应该这样”后，下一轮必须先重新输出需求回声，说明上一轮理解错在哪，并提出最多 3 个关键澄清问题或验证路径；确认前不得继续做同层补丁。
...
- **契约式小任务 + 运行时结构化分解 + 证据化验收**（按需启用，不是普通小任务默认流程）：
  - 只有用户要求严格流程 / 预计跨 8 个以上文件 / 高风险改动（架构、数据边界、安全、权限、支付、迁移、性能关键路径、破坏性操作）时，才在修改代码前创建或更新 `tasks/todo+<task-slug>.md`，把任务写成一个小 PR contract。
  - 普通小任务不要强制 contract。默认流程是：读相关文件，做最小必要改动，最终汇报改了什么。
  - 同类多文件修改时，第一轮先批量读取目标文件、相邻组件/样式和当前 diff；动手前形成“文件/模式 -> 修改动作”的修改映射。
  - 用户主动要求计划、验收清单、并行拆分、PR 级交付或审查证据时，也可以创建轻量 contract；L0 小改动最多只保留 `目标` 和 `验收`。
...
```
  ...
  - `允许路径`、`禁止路径` 必须来自用户、仓库配置、CI、package scripts、README 或已读源码；如果只是推断，写成 `建议允许路径`，不得伪装成硬约束。
  - `验收` 必须是可判断结果，不写“优化一下”“完善功能”这类不可验收表述。
  - contract 下方必须继续拆 `Epic -> Slice -> Step`；每个 Slice 写清目标、文件范围、依赖和完成证据。
  - Slice 必须尽量是垂直切片，能独立实现和 review；不要只按“前端/后端/文档”横切。
  - 发现 `范围` 或 `验收` 不准确时，先更新 contract，再继续实现。
  - contract 写入并开始执行后，该计划就是本轮完成契约。最终回复前必须逐项核对 Slice / Step、`验收`、`交付证据` 和 review 状态；完成、跳过、阻塞都要有证据。未完成项存在时，不得把任务描述为完成。
  - contract 写入后，不要求用户逐步确认；除非用户明确要求暂停/只讨论方案、存在关键歧义或触发高风险操作，否则按 slice 顺序自主执行、更新 checklist 并最终汇报证据。

- **多文件/并行任务使用 Subagent 隔离**：预计跨 8 个以上文件、PR 级交付或需要并行调研时，优先拆分为多个子任务；只有当前环境允许 Subagent 且用户明确允许并行/委托时，才交给 `agents/` 目录下专用 Subagent 执行（每个 Subagent 对应一个专注单一目标的 .md 文件）。如果系统规则禁止自动 spawn，则主 agent 按同一范围自行逐项执行，不得因此缩减范围。
  - 研究、调研、代码审查、文档编写等全委托 Subagent，主上下文仅做最终汇总，不被污染。
  - 主 agent 派发 Subagent 时，禁止使用会继承完整主上下文的 `fork_turns: "all"`。
  - 主 agent 必须显式写出子任务包：目标、单个 Slice、允许路径、禁止路径、验收项、交付证据、必须阅读的 md 文档清单。
...
- **完成前资深自检**：完成前列出「可能出问题的地方」。自问：「资深工程师会认可这个吗？」。永不主动标记 done。

### 非平凡任务与契约式任务的 Subagent 执行规则

**触发条件**：只有用户要求严格流程 / 预计跨 8 个以上文件 / 高风险改动 / PR 级交付 / 需要并行调研时，才创建或更新 `todo+<task-slug>.md` 并考虑 Subagent。环境允许 Subagent 且用户明确允许并行/委托时，才分配专用 Subagent 执行代码写入和实现工作，主 agent 负责最终 review、验收和整合。若系统规则禁止自动 spawn，主 agent 必须自行执行全部 Slice，不得把 Subagent 不可用当作拆版或缩减范围的理由。

**执行流程**：
1. 主 agent 完成需求澄清后，先创建/更新 contract（`todo+<slug>.md`），明确 `允许路径`、`禁止路径`、`验收`。
2. 在 contract 中或紧接着明确标注执行模式（推荐在 YAML 里加 `执行模式: subagent`、`main-agent` 或 `main-agent-fallback`）。
...
4. Subagent 必须严格遵守：
   ...
   - 输出变更文件列表和关键说明
5. 主 agent 接收 subagent 返回的摘要 + 变更后，严格执行“完成前资深自检” checklist：
   - 确认未触碰任何禁止路径
   - 对照 `验收` 逐条核对
   - 只有全部通过才更新 todo 状态并标记完成

**普通小任务**：单文件或少量文件、低风险、用户没有要求严格流程时，由主 agent 直接执行：读相关文件、最小改动、简洁汇报。
2. 任务分级与执行策略

- **L0（小改动）**：直接执行。
- **L1（多文件或常规开发任务）**：先做需求回声、列出步骤，再实施。
- **L2（高风险任务）**：先说明方案、影响和风险，确认后再实施。

执行时自动结合 Task Contract 触发条件；不要把普通 L1 小任务升级成强制 contract。
...
- `basic-memory`：跨会话、跨项目的长期知识库。
  ...
  - 不写临时任务状态、一次性 bug 细节、未确认猜测、敏感信息、token、`.env`、生产凭据。
...
| [.claude/rules/git-workflow.md](.claude/rules/git-workflow.md) | 完成前交付边界、提交规范、受保护操作 |
| [.claude/rules/hooks.md](.claude/rules/hooks.md) | ApexPowers loop hooks、PostToolUse 行数/安全门禁、轻量 Stop 安全门禁、trust 与 CI 分层 |
...
- 非平凡实现前必须做“需求回声”：用一句通俗中文复述“我理解你真正要的是……”，并把结果写成可验证行为；低风险明确任务可复述后直接执行。
- 如果同一句需求存在两个以上合理解释，或不同解释会影响交互模型、状态持久化、数据模型、权限、安全、范围边界或交付形式，先问用户确认，不要先写代码。
- 同一问题被用户反馈“不是这个意思 / 不对 / 还是不对 / 刷新后仍然 / 又出现 / 不应该这样”后，下一轮必须先重新输出需求回声，说明上一轮理解错在哪，并提出最多 3 个关键澄清问题或验证路径；确认前不得继续做同层补丁。
- 只有用户要求严格流程 / 预计跨 8 个以上文件 / 高风险改动 / PR 级交付时，才必须先创建或更新 `tasks/todo+<task-slug>.md`，用中文 YAML key 记录 `任务ID`、`目标`、`风险等级`、`范围/允许路径/禁止路径`、`验收`、`交付证据`、`需要审查`。
- 普通小任务不强制 contract；只要求读相关文件、做最小必要改动、汇报结果。
- 同类多文件修改时，第一轮先批量读取目标文件、相邻组件/样式和当前 diff；动手前形成“文件/模式 -> 修改动作”的修改映射。
- `允许路径`、`禁止路径` 不能凭空写；推断项必须标为 `建议允许路径` 或 `建议禁止路径`。
- contract 下方必须运行时拆成 `Epic -> Slice -> Step`，每个 Slice 都要有文件范围、依赖和完成证据。
- 最小交付指用最少改动完整满足用户明确验收，不代表只交 MVP、第一版或部分 Slice；除非用户明确同意拆版，否则不得缩减计划中的验收项。
- 一旦输出执行计划并开始实施，该计划就是本轮完成契约。最终回复前必须逐项核对 Slice / Step、验收、交付证据和必要 review；完成、跳过、阻塞都要有证据。
- 当环境允许 Subagent 且用户明确允许并行/委托时，跨 8 个以上文件、高风险或 PR 级交付可拆给 Subagent；若系统规则禁止自动 spawn，则主 agent 必须自行逐项执行，不得因此缩减范围。
...
- 完成必须同时满足验收、交付证据和必要 review；存在未完成项或有已知限制时必须如实说明，不得把任务描述为完成。
