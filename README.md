...
- `.codex/skills/apex-doctor`：只读检查 ApexPowers skills、agent mirrors、loop hooks、manifest 和 workflow state 的安装健康状态
- `.codex/skills/apex-lean-review`：显式审查当前 diff 或模块的过度工程、无必要依赖和平台原生替代机会
- `commands/apex-orchestrate-delivery.toml`：显式编排 worktree / issue / PR 级并行交付，串起 6 个角色模板、agent mirrors、`apex-to-issues` 和 Stop review request gate
- `.codex/skills/apex-session-init-codex`：Codex 开工前读取项目根 `AGENTS.md` / `CLAUDE.md` 并确认规则
- `.claude/skills/apex-session-init-claude-code`：Claude Code 开工前读取项目根 `CLAUDE.md` / `AGENTS.md` 并确认规则
...
Copy-Item "$ApexRoot\.claude\skills\apex-session-init-claude-code" "$Target\.claude\skills\" -Recurse -Force

New-Item -ItemType Directory -Force "$Target\.agents" | Out-Null
Copy-Item "$ApexRoot\.agents\*.md" "$Target\.agents\" -Force
```

如果目标项目也是私有仓库，可以提交 `.codex/skills/`、`.claude/skills/` 中复制过去的 skills 和 `.agents/`。
...
如果公开项目也希望保留官方前端、research 和 engineering workflow skills，可以只忽略 `.codex/skills/apex-init-*`、`.codex/skills/apex-sync-*`、`.codex/skills/apex-session-*` 与 `.claude/skills/apex-session-*`，继续提交 `frontend-design`、`gsap-*`、`react-best-practices`、`webapp-testing`、`web-design-guidelines`、`next-best-practices`、`github-solution-research`、`apex-grill-with-docs`、`apex-to-prd`、`apex-to-issues`。
...
## 安装到本机 Claude Code 全局 skills

适合希望同一台机器上的所有 Claude Code 项目都能使用 Apex session init，但不想把它复制进每个项目仓库。
...
Copy-Item "$ApexRoot\.claude\skills\apex-session-init-claude-code" "$ClaudeHome\skills\" -Recurse -Force
```
...
- Codex：`$CodexHome\config.toml` 中的 Apex 托管 `[[hooks.*]]` 块，与 `$CodexHome\hooks\apex_loop.py`
...
已有 `$CodexHome\config.toml` 会保留用户配置，只替换 Apex 托管块；已有 `$ClaudeHome\settings.json` 会保留用户 hook，并只替换 Apex 管理的 `apex_loop.py` 条目。既有 hook 脚本没有生成标记时默认跳过。用户明确要求覆盖/刷新时，加 `--force`。

重复安装是幂等的：再次运行同一条 `--write` 命令时，安装器会先移除旧的 Apex 管理条目，再写入当前版本，不会把同一组 hook 重复追加。
如果用户之前用旧版本安装过项目级 hooks，新的 agent-root 安装会迁移清理旧位置：删除项目 `.codex/hooks.json`、`.codex/config.toml`、`.claude/settings.json` 里的 Apex 条目，并删除带生成标记的项目级 runtime 副本；如果旧配置里混有用户自己的 hook，会保留用户 hook。
...
`--update` 是 manifest-aware reinstall；默认 dry run，带 `--write` 才落盘。`--uninstall` 只处理 manifest 记录的 Apex-managed 文件：Codex TOML 会删除 Apex 托管块并保留用户配置，Claude Code JSON 会 scrub Apex hook 条目并保留用户 hook；runtime 文件必须 hash 匹配、带生成标记或显式 `--force` 才删除；`tasks/loops/workflow.md` 与 `tasks/lessons.md` 是用户可编辑状态文件，卸载时默认保留。
...
如果你的 Codex 版本仍使用旧 `hooks.json`，可以显式加 `--codex-config-format json`。默认 `auto` 在 agent-root 安装时使用 `config.toml`，在旧项目级布局时仍使用 `hooks.json`。

安装后需要在 Codex 的 `/hooks` 或设置界面 review / trust 新 hook；Claude Code 侧会通过 agent 根目录的 `settings.json` 加载 hook。Loop hooks 只做确定性门禁：危险命令、secrets、写入后行数检查、`.agents` 镜像漂移提醒、Stop review request 与验证证据检查，不会在 hook 脚本里递归启动 agent。
...
```text
Use apex-session-init-codex before starting Codex work.
...
Use apex-doctor to check ApexPowers installation health without writing files.
Use apex-lean-review to review a diff for unnecessary abstractions, dependencies, custom code, and platform-native replacements.
Use apex-orchestrate-delivery to coordinate worktree-, issue-, or PR-level parallel delivery with role handoffs, apex-to-issues slicing, agent mirror checks, and Stop review request gates.
...
安装后，在目标项目中可以直接调用 Claude Code skill：

```text
/apex-session-init-claude-code
```

如果 `.claude/skills/` 目录是在 Claude Code 会话启动后首次创建的，重启 Claude Code 后再检查斜杆命令是否出现。

## 隐私注意事项

- 保持 GitHub 仓库为 private。
- 不要提交 `.serena/`、`.env`、token、凭据或机器本地状态。
- 对公开项目或客户项目，优先用全局安装，或者把 `.codex/skills/apex-*`、`.claude/skills/apex-*`、`.agents/`、`.codex/agents/`、`.claude/agents/` 加进 `.gitignore`。
- 如果用 token 拉取私有仓库，把 token 放进系统凭据管理器，不要写入脚本。
- 默认无遥测：ApexPowers hooks、doctor 和分发检查默认不发送 usage、prompt、源码片段、路径或错误报告。
- 供应链 / hook trust / NOTICE / sha256 manifest 规则见 `docs/supply-chain-trust-security.md` 和 `NOTICE.md`。
