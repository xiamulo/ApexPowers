...
flowchart TD
  ...
  I["Stop"] --> J["security / mirror drift / line hard limit block"]
  J --> K["默认不启用 ReviewGate / ValidationGate"]
  K --> L["允许完成；review 由显式流程或 CI 负责"]
```
...
| `PreToolUse` | `safety-write` | Edit/Write/MultiEdit/apply_patch 前 | 扫描即将写入的文本，阻断明显 secret-like 内容。 | 高风险阻塞。 |
| `PreToolUse` | `safety-read` | Read/Grep/Glob/mcp__.* 前 | 规范化路径、跟随既有 symlink，阻断 `.env`、credential、token、private-key 等敏感路径。 | 高风险阻塞。 |
| `PreToolUse` | `safety-shell` | Bash/Shell/PowerShell 前 | 拦截危险命令、破坏性 git、远程脚本管道执行，并扫描明显 shell 写入中的 secret-like 内容。 | 高风险阻塞。 |
| `PostToolUse` | `edit` | 写文件类工具后 | 对 payload 路径、apply_patch 路径或 git diff fallback 做 secret 内容与有效行数检查，并提醒 mirror drift。 | 不能撤销已完成工具；secret 写入会反馈并标记 `security_required`，行数/mirror 问题最终由 Stop gate 执行。 |
| `PostToolUse` | `bash` | shell 工具后 | 识别 shell redirect/PowerShell 写入目标；没有明确路径时回退到 git changed files。 | 同上。 |
| `PostToolBatch` | `default` | 批量工具调用后 | 复用 diff-aware PostToolUse 检查路径，配合 guard cache / failure dedupe 降低重复扫描。 | 同上。 |
| `PreCompact` | `default` | context compaction 前 | 写入 `tasks/loops/session-snapshot.json`，保留 workflow、active task、diff hash、review path 和 blockers。 | 不阻塞。 |
| `Stop` | `default` | agent 准备结束 turn 时 | 阻塞 unresolved security cleanup、agent mirror drift、行数硬上限。默认不阻塞 review、validation 或 stale diff。 | 可阻塞安全/一致性问题。 |
...
| `SecretContentGuard` | `apex_loop_runtime.py` | 写入内容中疑似 private key、OpenAI key、GitHub PAT、AWS key、Slack token 等。 | `PreToolUse` 写前阻断；`PostToolUse` 作为已执行工具的兜底反馈。 |
| `LineLengthGuard` | `apex_loop_runtime.py` + `apex_loop_utils.py` | 按文件类型计算有效代码行数；frontend component、script/backend module 有不同 warning/hard 阈值，generated/vendor/fixture 等例外。 | `PostToolUse` warning；`Stop` 对新增或跨过硬阈值的源码文件 block。 |
| `MirrorDriftGuard` | `apex_loop_runtime.py` | `.agents/*.md` 变化但 `.codex/agents/*.toml` / `.claude/agents/*.md` 未一起变化。 | `PostToolUse` warn，`Stop` block。 |
| `ReviewGate` | `apex_loop_runtime.py` + `apex_loop_utils.py` | 保留的 strict-mode 实现；默认 Stop 不实例化、不调用，也不创建 review request。 | 显式 strict workflow。 |
| `ValidationGate` | `ReviewGate` 内部 | 保留的 strict-mode 实现；默认 Stop 不检查 validation frontmatter。 | 显式 strict workflow。 |
| workflow-state 推导 | `apex_loop_utils.py` | 根据 todo、security state、diff、state.json 推导 `no_task`、`planning`、`implementing`、`security_required`、`done` 等。默认代码 diff 进入 `implementing`。 | `SessionStart` 和 `UserPromptSubmit`。 |
...
| `tasks/loops/active.json` | 用户 / 编排器 / agent | hook runtime | 多任务并行时显式列出 active tasks、owned_paths、review_path 和 risk_level，避免误绑定“最新 todo”。 |
| `tasks/loops/security-required.json` | PostToolUse secret guard | hook runtime、agent | 记录已执行工具产生 secret-like 内容后的清理要求；存在时 workflow 进入 `security_required` 且 Stop 阻塞完成。 |
| `tasks/loops/session-snapshot.json` | PreCompact hook | agent / 用户 | compaction 前保存 workflow、active task、diff hash、review path 和 blockers。 |
| `tasks/reviews/<slug>.md` | 显式 review 流程或旧 strict gate 生成，reviewer / agent 后续更新 | doctor、agent、显式 strict workflow | 审查请求、scope、required reviewer、completion contract。默认 Stop 不自动创建。 |
...
| `security_required` | 已完成工具产生 secret-like 内容，需要清理。 | 移除内容、轮换真实凭据、重新验证后再尝试完成。 |
| `review_required` | 兼容旧 state 文件或显式 strict workflow；默认不会仅因代码 diff 进入。 | 调用 `code-reviewer`，更新 `tasks/reviews/<slug>.md`。 |
| `validation_required` | 兼容旧 state 文件或显式 strict workflow；默认不会仅因 review frontmatter 进入。 | 运行约定测试，把结果写入 review 文件。 |
| `done` | 显式流程完成，或人工/CI 已确认可以结束。 | 可以结束当前任务或进入提交流程。 |

Review 文件的完成合约是显式 review / strict workflow 的边界。当前 runtime 保留读取 Markdown 文件开头结构化 frontmatter 的能力；新 review request 使用 YAML，旧 TOML frontmatter 兼容读取。正文只作为人类 notes，不参与放行：

- `status: ready`。
- `validation: pass` 或 `validation: automated-pass`。
- `reviewed_diff_hash` 必须覆盖当前代码 diff，或 `reviewed_file_hashes` 必须逐文件匹配。
- `validation_evidence.required_checks` 必须全部记录 `exit_code = 0`。
- `reviewer.role` 对普通风险必须是 `human`、`independent-agent` 或 `ci`；高风险必须是 `human` 或 `ci`。
- `reviewer.role: same-agent` 或 `reviewer.id == implementer.id` 会 fail closed。
- 如果结构化 findings 里仍有 open critical/warning，或 review Ready 之后又产生新的代码 diff，显式 strict workflow 应继续阻止完成。默认 Stop 不执行这些 review/validation 检查。
...
- 大交付完成前仍要满足显式 review gate：`code-reviewer` Ready、`Validation: Pass`、命令摘要、Ready 后无新代码 diff。
...
| 测试文件 | 覆盖内容 |
| --- | --- |
| `tests/test_apex_loop_hooks.py` | route render、TOML config、UserPromptSubmit advisory、SessionStart context、workflow state、危险命令、secret path/content、行数阈值、mirror drift、默认 Stop 不强制 review/validation。 |
...
| 并行交付怎么用 | “并行交付协议”说明 worktree / issue / PR 级编排、role dispatch 和显式 review gate。 |
