...
- **多文件/并行任务使用 Subagent 隔离**：修改 >3 个文件或需要并行调研/验证时，优先拆分为多个子任务；只有当前环境允许 Subagent 且用户明确允许并行/委托时，才交给 `agents/` 目录下专用 Subagent 执行（每个 Subagent 对应一个专注单一目标的 .md 文件）。如果系统规则禁止自动 spawn，则主 agent 必须按同一 contract 自行逐项执行，不得因此缩减范围。
...
**触发条件**：只要任务需要创建或更新 `todo+<task-slug>.md`（即 L1/L2 非平凡任务、需要 contract 的任务），当环境允许 Subagent 且用户明确允许并行/委托时，优先分配专用 Subagent 执行代码写入和实现工作，主 agent 负责最终 review、验收和整合。若系统规则禁止自动 spawn，主 agent 必须按同一 contract 自行执行全部 Slice，不得把 Subagent 不可用当作拆版或缩减范围的理由。
...
- 当环境允许 Subagent 且用户明确允许并行/委托时，非平凡任务优先拆给 Subagent；若系统规则禁止自动 spawn，则主 agent 必须按同一 contract 自行逐项执行，不得因此缩减范围。
