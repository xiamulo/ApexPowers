...
上下文隔离硬规则：主 agent 派发子智能体时不得使用 `fork_turns: "all"` 继承完整主上下文，必须显式给出目标、单个 Slice、允许/禁止路径、验收、必跑检查、交付证据和必须阅读的 md 文档清单。子智能体默认是叶子执行者，不能再 spawn / fork / 调度新的 Subagent；需要二次拆分或更多上下文时交回主 agent。
