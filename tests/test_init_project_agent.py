...
class InitProjectAgentTests(unittest.TestCase):
    ...
    def test_generated_roots_include_chinese_task_contract(self) -> None:
        ...
        for content in (claude_md, agents_md):
            ...
            self.assertIn("不代表只交 MVP、第一版或部分 Slice", content)
            self.assertIn("用户要求严格流程 / 预计跨 8 个以上文件 / 高风险改动", content)
            self.assertIn("普通小任务不要强制 contract", content)
            self.assertIn("读相关文件，做最小必要改动，最终汇报改了什么", content)
