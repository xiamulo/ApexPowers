...
完成契约硬规则：最小交付指用最少改动完整满足用户明确验收，不代表只交 MVP、第一版或部分 Slice。一旦输出执行计划并开始实施，该计划就是本轮完成契约；最终回复前必须逐项核对 Slice / Step、验收、必跑检查、交付证据和 review 状态，未完成项存在时不得把任务描述为完成。
上下文隔离硬规则：只有环境允许 Subagent 且用户明确允许并行/委托时，非平凡任务才优先拆给 Subagent；若系统规则禁止自动 spawn，主 agent 必须按同一 contract 自行逐项执行，不得因此缩减范围。主 agent 派发子智能体时不得使用 `fork_turns: "all"` 继承完整主上下文，必须显式给出目标、单个 Slice、允许/禁止路径、验收、必跑检查、交付证据和必须阅读的 md 文档清单。子智能体默认是叶子执行者，不能再 spawn / fork / 调度新的 Subagent；需要二次拆分或更多上下文时交回主 agent。
