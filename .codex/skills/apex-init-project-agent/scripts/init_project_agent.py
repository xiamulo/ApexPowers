...
- 以最小交付为准，不擅自扩范围；最小交付指用最少改动完整满足用户明确验收，不代表只交 MVP、第一版或部分 Slice。除非用户明确同意拆版，否则计划中的所有验收项和必跑检查完成前不得 final；只能在阻塞时汇报 blocker。
- 不要写成命令式、审查式、技术汇报式语气。
...
- **契约式小任务 + 运行时结构化分解 + 证据化验收**（按需启用，不是普通小任务默认流程）：
  ...
  - 普通小任务不要强制 contract。默认流程是：读相关文件，做最小必要改动，运行最小验证，最终汇报改了什么和验证结果。
  - 用户主动要求计划、验收清单、并行拆分、PR 级交付或审查证据时，也可以创建轻量 contract；L0 小改动最多只保留 `目标`、`验收` 和最小验证。
...
```
  ...
  - contract 写入并开始执行后，该计划就是本轮完成契约。最终回复前必须逐项核对 Slice / Step、`验收`、`必跑检查`、`交付证据` 和 review 状态；完成、跳过、阻塞都要有证据。未完成项存在时，不得把任务描述为完成。
  - contract 写入后，不要求用户逐步确认；除非用户明确要求暂停/只讨论方案、存在关键歧义或触发高风险操作，否则按 slice 顺序自主执行、更新 checklist、运行验证并最终汇报证据。

- **多文件/并行任务使用 Subagent 隔离**：预计跨 8 个以上文件、PR 级交付或需要并行调研/验证时，优先拆分为多个子任务；只有当前环境允许 Subagent 且用户明确允许并行/委托时，才交给 `agents/` 目录下专用 Subagent 执行（每个 Subagent 对应一个专注单一目标的 .md 文件）。如果系统规则禁止自动 spawn，则主 agent 按同一范围自行逐项执行，不得因此缩减范围。
...
**触发条件**：只有用户要求严格流程 / 预计跨 8 个以上文件 / 高风险改动 / PR 级交付 / 需要并行调研验证时，才创建或更新 `todo+<task-slug>.md` 并考虑 Subagent。环境允许 Subagent 且用户明确允许并行/委托时，才分配专用 Subagent 执行代码写入和实现工作，主 agent 负责最终 review、验收和整合。若系统规则禁止自动 spawn，主 agent 必须自行执行全部 Slice，不得把 Subagent 不可用当作拆版或缩减范围的理由。
...
**普通小任务**：单文件或少量文件、低风险、用户没有要求严格流程时，由主 agent 直接执行：读相关文件、最小改动、最小验证、简洁汇报。
2. 任务分级与执行策略
...
| [.claude/rules/hooks.md](.claude/rules/hooks.md) | ApexPowers loop hooks、PostToolUse 行数/安全检查、轻量 Stop 安全门禁、trust 与 CI 分层 |
...
- 只有用户要求严格流程 / 预计跨 8 个以上文件 / 高风险改动 / PR 级交付时，才必须先创建或更新 `tasks/todo+<task-slug>.md`，用中文 YAML key 记录 `任务ID`、`目标`、`风险等级`、`范围/允许路径/禁止路径`、`验收`、`必跑检查`、`交付证据`、`需要审查`。
- 普通小任务不强制 contract；只要求读相关文件、做最小必要改动、运行最小验证、汇报结果。
...
- 一旦输出执行计划并开始实施，该计划就是本轮完成契约。最终回复前必须逐项核对 Slice / Step、验收、必跑检查、交付证据和必要 review；完成、跳过、阻塞都要有证据。
...
- 完成必须同时满足验收、必跑检查、交付证据和必要 review；未验证、存在未完成项或有已知限制时必须如实说明，不得把任务描述为完成。
