...
routingDescription: 适用：卡顿、慢、内存高、主线程阻塞、WebGL/Canvas 资源异常、导入导出慢或性能风险审查。不适用：普通代码审查、纯 UI 文案、需求规划、无性能症状的泛泛优化。
context: fork
tools:
  - Read
  - Grep
...
- 当前文件是 ApexPowers 私有 `.agents/perf-optimizer.md` 模板，保留现有 Markdown + YAML frontmatter。
- 如果要让 Codex 官方子智能体直接加载，建议镜像为 `.codex/agents/perf-optimizer.toml`，并把正文合并进 `developer_instructions`；默认建议 `sandbox_mode = "read-only"`，只有用户明确要求修复时才交给 developer/implementer。
- 如果要让 Claude Code 官方子智能体直接加载，建议镜像为 `.claude/agents/perf-optimizer.md`，保留只读工具和性能相关 MCP。
