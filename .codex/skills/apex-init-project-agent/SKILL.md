...
如果用户明确要求“重新生成 / 重跑 / 覆盖 / 刷新 / regenerate”，就进入重新生成模式：根 `AGENTS.md` 必须用固定模板重写，所有 rules 文件也必须重新读取源码后重写，不能因为文件已存在就跳过。使用脚本时传入 `--force`、`--all` 或 `--regenerate` 获取全量 rules 清单。
...
只有在用户明确要求重新生成时，才使用 `--force` 覆盖根 `AGENTS.md` 和已有 rules 文件。
