...
第一条是 dry run。确认将要创建的文件合理后，再运行带 `--write` 的命令。`AGENTS.md` 是 Codex 核心规则入口，内联关键硬规则，并从 `api-design.md`、`backend.md`、`frontend.md` 各提炼最多 10 行项目专项硬规则；`CLAUDE.md` 保留 Claude Code 的 `.claude/rules` 分层入口。
